...
NF_OUT_DIR=output
TO_MERGE_ALL_PCAP="n" # y/n
MERGED_NF_FILENAME="merged.nf"
...
start_netflow_capuring_process() {
  ...
  nfcapd -p 9995 -w $BIN_OUT_DIR >/dev/null &
}
...
merge_and_convert_pcap_to_netflow() {
  # xxx.pcap -> merged.pcap
  echo "(Stage 1/3) Convert $(echo "$1" | tr ' ' '\n' | wc -l) PCAP files to merged.pcap"
  echo "First 12 files:"
  echo "$1" | tr ' ' '\n' | head -12
  mergecap -w merged.pcap $1

  # merged.pcap -> nfcapd.*
  echo "(Stage 2/3) Convert merged.pcap to $BIN_OUT_DIR/nfcapd.*"
  convert_single_pcap_to_nfcapd merged.pcap

  # nfcapd.* -> merged.nf
  nf_path=$NF_OUT_DIR/$MERGED_NF_FILENAME
  echo -e "\n\n\n(Stage 3/3) Convert $BIN_OUT_DIR/nfcapd.* to $nf_path"
  convert_nfcapd_to_netflow $BIN_OUT_DIR $nf_path
  rm -f $BIN_OUT_DIR/*
